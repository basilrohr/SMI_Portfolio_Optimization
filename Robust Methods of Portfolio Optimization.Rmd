...
abstract: "Markowitz optimization requires estimates of mean return, standard deviation and correlation of individual assets, all of which are subject to estimation errors. This leads to the risk of overfitting historical data and, in the case of portfolio optimization, creating portfolios that perform well in backtesting but poorly in the future. To overcome this problem, several robust approaches are investigated, such as different asset grouping methods and shrinkage factors for mean return and correlation. The goal is to provide a recommended approach for optimizing a portfolio that includes all assets from the Swiss Market Index. In summary, three key observations are made. First, asset grouping leads to significant noise reduction and thus better portfolio performance compared to individual assets. Likewise, shrinkage leads to increased performance due to major noise reduction. Using both methods simultaneously achieves a further increase in performance. The entire code is published on \\url{https://github.com/pascalaigner/markovitz} and the Shiny app can be accessed at \\url{https://project-wi.shinyapps.io/application}.\\par\\textbf{Keywords:} Portfolio optimization, Markovitz model, Sharpe ratio, Robust methods, In-sample, Out-of-sample, Asset grouping, Shrinkage\\newpage"
output:
  bookdown::pdf_book:
    toc: false
...
# Introduction
In 1952, the economist Harry Markovitz pioneered modern portfolio theory. He developed a mathematical model to calculate efficient portfolios for given return and risk. The key point when selecting assets is, that not only their individual performance should be considered, but also how their correlation affects the portfolio. The input for Markovitz' optimization is historical data, which is subject to estimation errors. These lead to the creation of portfolios which perform well in backtesting but poorly in the future.

The fundamental question is how to create a model from historical data that delivers robust results on independent data. In this paper, the data used are the return time series of all assets included in the Swiss Market Index from the past 20 years. The standard error of mean returns and volatilities are calculated, to provide insights into which parameters should be investigated further. Afterwards, a short review of Markovitz' portfolio theory, mainly the minimum variance and tangency portfolio, is presented.

Several approaches are investigated to achieve a robust optimization. The first method described is asset grouping, where a qualitative and a quantitative approach are applied and compared. Followed by a description of bootstrapping, which is used to determine the standard deviation of the asset weights. To measure the performance on independent data, cross validation is applied to obtain an out-of-sample time series. As a final approach, shrinkage of mean returns and correlations is discussed.

These methods are applied to the historical data and their performance in terms of Sharpe ratio is compared. The in- and out-of-sample performances are evaluated and the optimal shrinkage factors are identified. At the end of this paper, the aim is to provide an approach to create a robust portfolio optimization for all assets included in the Swiss Market Index.
...
## Logarithmic return
The reason for calculating logarithmic returns is that stock returns are generally assumed to be log-normally and not normally distributed. In later calculations, the risk-free rate of return has to be subtracted from the return. For the sake of simplicity, this is already done from the start as illustrated in eq. \@ref(eq:logr). The \mbox{LIBOR} is commonly used, for the risk-free rate $R_{f}$ shown in table \@ref(tab:annrftab). Due to several negative incidents in the past, the Swiss Average Rate Overnight (SARON) is used for the time period 2014 to 2020. The SARON is based on daily transactions and is therefore considerably more transparent than the LIBOR. [@saron], [@libor]
...
The volatility in eq. \@ref(eq:vol) describes the risk of an asset and is a statistical measurement of the dispersion of its returns. In general, the higher the volatility, the riskier the asset.
...
### t-Test
A t-test for the correlation as in eq. \@ref(eq:ttest) is conducted to investigate if the true correlation is significantly different to zero.
...
The null hypothesis $H_{0}$ says that the correlation is not significantly different to zero and the alternative hypothesis $H_{1}$ that it is significantly different to zero.
...
With the help of the t-distribution in figure \@ref(fig:tdist) one can clearly see that a value of approximately two is already below the level of \mbox{$\alpha = 0.05$}. Therefore, with the obtained test statistic value of `r tstatistic` which is on the far right in figure \@ref(fig:tdist), the p-value is almost zero. Consequentially, the null hypothesis can be rejected, meaning that the true correlation is significantly different to zero.
...
Figure \@ref(fig:serplot) depicts the standard error of the mean return for each stock. It is seen clearly that stocks such as *Nestle* and *Swisscom* with a low volatility in table \@ref(tab:rmrvoltab) also have a lower standard error. Likewise, *ABB* and *Credit Suisse* with a high volatility show a higher standard error.
...
The efficient frontier is a set of points that extends in the return-volatility diagram illustrated in figure \@ref(fig:refplot) between the MVP at the left edge of the reachable area and the TP. All portfolios above the MVP along this line are efficient because they have the highest possible return at a defined level of volatility.
...
In figure \@ref(fig:refplot) one can see that the efficient frontier covers the range in which all possible portfolios are located. The TP achieves a significantly higher return than the stocks individually. The MVP is at the very left of the achievable area of the efficient frontier which means that there is no portfolio with a lower volatility.

\newpage

## Sharpe ratio {#sharpe}
The Sharpe ratio measures the performance of an asset, meaning its return compared to its risk. Generally, the greater the value of the Sharpe ratio, the more attractive the asset. In practice, the Sharpe ratio is not only positively received. In this paper however, the Sharpe ratio is relied on because it is a simple performance indicator. It is calculated by the return in excess of the risk-free rate per unit of volatility. As daily historical data is used, the Sharpe ratio is annualized by a multiplication with $\sqrt{252}$. [@sharpe]
...
<p> *$R$ = return* <p>
<p> *$R_{f}$ = risk-free rate of return* <p>
<p> *$\sigma$ = volatility of return* <p>

# Methodology {#methodology}

## Grouping {#grouping}
Grouping of assets is an approach used to reduce the noise given by individual assets and therefore increase their performance. A qualitative and a mathematical procedure are applied.
...
kable(gr_mr_vol, col.names = c("Group","Return [%]", "Volatility [%]"),
      caption = "Mean return and volatility of groups by industry",
      digits = 3, booktabs = T, linesep = "")
```
...
Figure \@ref(fig:gefplot) gives some visual intuition. The groups contain stocks with similar volatility and return, so it makes sense that *ABB* is grouped with *UBS* and not with *Geberit*.
...
gr_mr_vol = data.frame(colnames(g2r[-1]), mean_returns(g2r), volatilities(cov_mat(g2r)),
                       row.names = NULL)
kable(gr_mr_vol, col.names = c("Group","Return [%]", "Volatility [%]"),
      caption = "Mean return and volatility of groups by correlation",
      digits = 3, booktabs = T, linesep = "")
```
...
Model three in figure \@ref(fig:tpwshrinkbymodel) shows such extreme asset weights ranging from approximately -20 to 20. The asymptote is visualized as a vertical red line. It is not optimal when such extreme weights are applied to returns as done in the cross validation. This can negatively influence further calculations and lead to undesired results. The other four models show asset weights in a normal scale.
...
# Results
The results of the different approaches described in chapter \@ref(methodology) are presented and compared by their Sharpe ratio in this chapter.
...
Figure \@ref(fig:mvpsegr) illustrates the mean weights and their standard deviation of the MVP and figure \@ref(fig:tpsegr) of the TP, both grouped by industry.

```{r mvpsegr, fig.cap = "MVP weights with standard deviation by group by industry", fig.height = 3}
gg_errorbar(colnames(gr[-1]), mvpw_gr, mvpw_sd_gr, c(minmvp, maxmvp), "Weights",
            theme = custom_theme_markdown)
```

```{r tpsegr, fig.cap = "TP weights with standard deviation by group by industry", fig.height = 3}
gg_errorbar(colnames(gr[-1]), tpw_gr, tpw_sd_gr, c(mintp, maxtp), "Weights",
            theme = custom_theme_markdown)
```

The same conclusion can be drawn for the grouped stocks and the individual stocks in chapter \@ref(res1). The TP weights standard deviation is also much larger than the one of the MVP.

In table \@ref(tab:srcomtab2), the in- and out-of-sample Sharpe ratios for both groups are shown.
...
In chapter \@ref(groupclust), cluster analysis is used to optimize the group composition. Higher correlating stocks are grouped together which results in a lower correlation between the individual groups. Therefore, a better diversified portfolio is achieved with the groups by correlation compared to the groups by industry. This also leads to a higher in- and out-of-sample Sharpe ratio.
...
When looking at the SMI constituents curve, it is seen that the highest Sharpe ratio is achieved with a shrinkage factor of 0.05. If past returns are taken too seriously (higher shrinkage factor) or not considered at all (shrinkage factor zero), the out-of-sample Sharpe ratio drops. So, the returns can be trusted only 5%, meaning that they have only little impact on the optimization of the weights. This result is plausible, as the returns are very noisy. Further can be seen that the Sharpe ratio has a sharp bend and drop below zero. This is caused by the close to zero denominator of the tangency portfolio eq. \@ref(eq:tpw) as described in chapter \@ref(shrinkage).
...
kable(tpw_r, col.names = c("Stock", "Before shrinkage", "After shrinkage"),
      caption = "SMI constituents tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```

\newpage

When comparing the two sets of weights, a significant difference can be seen. Overall, the weights after shrinkage are closer around zero than they were before. This is caused by the fact that the returns are taken into account only 5%, meaning that they have only little impact on the optimization of the weights. Therefore, the weights are much closer to the MVP weights as seen in table \@ref(tab:mvpwrtab) in chapter \@ref(markovitz).

Compared to the weights of the individual stocks already shown in table \@ref(tab:tpwrtab), the weights of the groups in table \@ref(tab:srcomtab2) and \@ref(tab:srcomtab3) are significantly higher or lower. To recall, the weights must summed up to one according to constraint \@ref(eq:constraint1). If the groups perform differently, like it is the case for *Finance* (negative mean return) and *Pharma* (positive mean return) in the groups by industry, the weights drift apart. This becomes even more prominent in the groups by correlation between *Group 1* and *Group 3*. As mentioned in chapter&nbsp;\@ref(markovitz), negative weights correspond to short sales. The difference between the weights before and after shrinkage is insignificant. This is attributed to the shrinkage factor of the mean returns being one and the one for the correlation being 0.95. So there is almost no shrinkage applied.
...
kable(tpw_gr_df, col.names = c("Group", "Before shrinkage", "After shrinkage"),
      caption = "Groups by industry tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
...
kable(tpw_g2r_df, col.names = c("Group", "Before shrinkage", "After shrinkage"),
      caption = "Groups by correlation tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
...
# Conclusion
Considered in isolation, the highest increase of Sharpe ratio is achieved with the grouping method by correlation. However, the highest Sharpe ratio is obtained with a combination of grouping and shrinkage. It should be noted that when one method is applied after the other, its influence on the Sharpe ratio is significantly lower. This has to do with the fact that the noise can only be reduced to a certain degree. In this case, the first method which is grouping, has a larger increase in Sharpe ratio, while shrinkage, as the second method, has only a small additional increase. However, it is well apparent that for the SMI constituents where only shrinkage is applied, the rise in Sharpe ratio is similarly high compared with grouping as the first method.

With these insights, the best procedure for optimizing multiple assets can be derived. In a first step, one would group by correlation to reduce noise by a significant amount. The second step would be applying shrinkage to mean returns and correlation. Examining the groups implies that returns remain unshrunk, while correlation is shrunk by a factor of 0.95. Although this is not generally valid, overall it can be said that shrinkage offers only a small gain in addition to the previous grouping. But if grouping is omitted, it is possible that a higher Sharpe ratio can be achieved with the shrinkage approach, especially with a high number of assets.

To give the final result, the weights in the after shrinkage column from table \@ref(tab:srcomtab3) obtain the best future performance for a portfolio including all assets in the Swiss Market Index.
