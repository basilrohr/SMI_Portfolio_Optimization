...
gg1 = gg_errorbar(factor(stocks[order], levels = stocks[order]), mr[order], se_r[order], c(min, max), "Return [%]", theme = custom_theme_markdown)
...
```{r gg3, fig.cap = 'Bootstrap samples efficiency frontier', fig.height = 6}
...
efw_gr = ef_weights(mvp_weights(gcm), tp_weights(gcm, gmr), seq(-2, 2, 0.1))
efp_gr = ef_points(efw_gr, gcm, gmr)

efp = data.frame(n = c(stocks, names(groups)), vol = c(volatilities(cm), volatilities(gcm)), mr = c(mr, gmr), row.names = NULL)

g = c()
for (i in efp$n) {
  g = c(g, names(which(unlist(groups) == i)))
}
efp$g = c(substr(g, 1, nchar(g)-1), names(groups))
...
suppressMessages(suppressWarnings(print(ggplot() +
  ...
  geom_text(aes(x = tp_point(efp_r)[1], y = tp_point(efp_r)[2], label = "SMI"),
            vjust = -2) +
  geom_path(aes(x = efp_gr[,1], y = efp_gr[,2]), alpha = 0.5) +
  geom_point(aes(x = mvp_point(efp_gr)[1], y = mvp_point(efp_gr)[2], color = "MVP")) +
  geom_point(aes(x = tp_point(efp_gr)[1], y = tp_point(efp_gr)[2], color = "TP")) +
...
gg3 = gg_errorbar(factor(stocks[order], levels = stocks[order]), mvpw_r[order], mvpw_sd_r[order], c(min, max), "Weights", theme = custom_theme_markdown)
...
gg3 = gg_errorbar(factor(stocks[order], levels = stocks[order]), tpw_r[order], tpw_sd_r[order], c(min, max), "Weights", theme = custom_theme_markdown)
