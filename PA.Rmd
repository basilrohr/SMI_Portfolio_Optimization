...
plots12 = align_plots(gg1, gg2, align = "h")
```
...
kable(se_volr, col.names = c("Stock", "Standard error [%]"), caption = "Standard error of volatility by SMI", digits = 5, booktabs = T, linesep = "")
```

```{r tabplot6}
se_volgr = data.frame(colnames(gr[-1]), apply(gr[,-1], 2, se_sd), row.names = NULL)
kable(se_volgr, col.names = c("Group", "Standard error [%]"), caption = "Standard error of volatility by group", digits = 5, booktabs = T, linesep = "")
```
...
gg5 = gg_errorbar(factor(stocks[order], levels = stocks[order]), tpw_r[order], tpw_sd_r[order], c(min, max), "Weights", theme = custom_theme_markdown)
