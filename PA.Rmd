...
```{r tpwgrtab}
tpw_gr_df = data.frame(colnames(gr[-1]), tp_weights(cov_mat(gr), mean_returns(gr)))
kable(tpw_gr_df, col.names = c("Group", "Weight"), caption = "Groups by industry in sample tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```

```{r tpwg2rtab}
tpw_g2r_df = data.frame(colnames(g2r[-1]), tp_weights(cov_mat(g2r), mean_returns(g2r)))
kable(tpw_g2r_df, col.names = c("Group", "Weight"), caption = "Groups by correlation in sample tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
