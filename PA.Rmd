...
The weights before shrinkage in the following table \@ref(tab:tpwrstab) can be found in the previous chapter \@ref(markovitz) in table \@ref(tab:tpw).
```{r tpwrstab}
mr = mean_returns(r)
smr = shrinkage_mr(mr, 0.05)

tpw_r = data.frame(colnames(r[-1]), tp_weights(cov_mat(r), mr), tp_weights(cov_mat(r), smr))
kable(tpw_r, col.names = c("Stock", "W. before shrinkage", "W. after shrinkage"), caption = "Tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```

When comparing the two sets of weights a significant difference can be seen. Overall the weights after shrinkage are closer too zero as they were before. This is caused by the fact that the returns are taken into account for only 5%. Therefore the weights are much closer to the MVP, seen in table \@ref(tab:mvpw).
...
kable(tpw_gr_df, col.names = c("Stock", "W. before shrinkage", "W. after shrinkage"), caption = "Groups by industry in sample tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
...
kable(tpw_g2r_df, col.names = c("Stock", "W. before shrinkage", "W. after shrinkage"), caption = "Groups by correlation in sample tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
