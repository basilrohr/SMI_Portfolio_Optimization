---
title: "Robust Methods of Portfolio Optimization, Exemplified by the Swiss Market Index"
subtitle: "ZHAW School of Engineering, Winterthur"
author: 
- Pascal Aigner 
- Maurice Gerber
- Basil Rohr
...
load("./Data/rf_1d.Rda")
load("./Data/SMI_returns_1d.Rda")
R.utils::sourceDirectory("./Functions", modifiedOnly = F)
```
...
# Swiss Market Index
The Swiss Market Index or (SMI) is the most important stock index in Switzerland and contains the 20 largest companies traded on the Swiss stock exchange. The SMI covers approximately 80% of the total capitalization of the Swiss stock market. It is also a price index, which means that dividends are not included in the index. The SMI is always reviewed twice a year and, if necessary, reassembled. In our case, we use the composition per end of 2018. The reason for the 2018 composition is, that enough historical data for all those companies is available.     
For the calculations of the daily logarithmic return \@ref(eq:LogR) and volatility \@ref(eq:Volatility) we use historical data from `r format(df[,1][1], "%d.%m.%Y")` to `r format(df[,1][nrow(df)], "%d.%m.%Y")`. The results are shown in table \@ref(tab:tabplot2). [1] [2]
...
```{r tabplot1, echo = F}
Rftap = rf * 260
Rftap = data.frame(names(Rftap), round(Rftap, 3))
rownames(Rftap) = c()
kable(Rftap, caption = "Average overnight risk-free return per year in \\%", format = "latex", 
      booktabs = T, linesep = "", col.names = c("Year", "Rate [%]"))
```
...
kable(SMI_mu_sd, caption = "SMI with expected return and volatility", digits = 3, format = "latex",
      col.names = c("Stock","Return [%]", "Volatility [%]"), booktabs = T, linesep = "")
```
...
gg_cor(cor(SMI_returns[,-1]), 2, 8, theme = custom_theme_markdown)
```
...
```{r, echo = F}
mincor = round(min(abs(cor(SMI_returns[,-1]))), 3)
mincorname = row.names(which(round(cor(SMI_returns[,-1]),3) == mincor, arr.ind = TRUE))
tvalue = round(cor.test(SMI_returns[,-1][,mincorname[1]], SMI_returns[,-1][,mincorname[2]])[[1]], 3)
```
...
\begin{equation}
  t = \frac{`r mincor`*\sqrt{`r nrow(SMI_returns)`-2}}{\sqrt{1 - `r mincor`^2}} = `r tvalue`
  \label{eq:secr}
\end{equation}

The students t-distribution with `r nrow(SMI_returns)` degrees of freedom tells us that the probability of getting a test-statistic in the interval of [-`r tvalue`,`r tvalue`] equals 1. Therefore, the probability of getting a test-statistic out of the interval equals 0.
...
test1 = as.matrix(SMI_returns[,-1]) %*% weights_SMI
sr_SMI = sharpe_ratio(mean(test1), sd(test1))
sr_SMI = round(sr_SMI, 3)
```

Before any weighting of a portfolio is optimized, the Sharpe ratio of the SMI is calculated. This is done by weighting all stocks equally, 1/20 each, and then applying the weightings to the actual returns of the historical data. This results in a Sharpe ratio of `r sr_SMI`. Note that the Sharpe ratio is annualized, which means that it is multiplied by $\sqrt{252}$. As mentioned above, the Sharpe ratio is used as a measure of the optimization and robustness. A robust optimization leads to a lower volatility which results in a higher Sharpe ratio. The higher the value the better the optimization.
...
```{r tabplot3, echo = F}
kable(SMI_groups_mu_sd, caption = "groups with expected return and volatility", format = "latex", digits = 3, booktabs = T, linesep = "")
```
...
```{r, echo = F, fig.height = 6}
gg_cor(cor(SMI_groups_returns[,-1]), 4, 12, theme = custom_theme_markdown)
```
...
test2 = as.matrix(SMI_groups_returns[,-1]) %*% weights_groups
sr_groups = sharpe_ratio(mean(test2), sd(test2))

t4 = data.frame(t(c(sr_SMI, sr_groups)))
 
kable(t4, caption = "sharp ratio", format = "latex", 
...
gg1 = gg_errorbar(SMI_mu_sd$Stock, SMI_mu_sd$`Mean return [%]`, se_mean_SMI, c(min, max), 90, "return in %", theme = custom_theme_markdown)

gg2 = gg_errorbar(SMI_groups_mu_sd$Group, SMI_groups_mu_sd$`Mean return [%]`, se_mean_groups, c(min, max), 0, "return in %", theme = custom_theme_markdown)

pl = align_plots(gg1, gg2, align="h")
```

```{r gg1, echo = F, fig.cap = 'Expected mean return with standard error by stock', fig.height = 4, warning = F, message = F}
ggdraw(pl[[1]])
```

```{r gg2, echo = F, fig.cap = 'Expected mean return with standard error by group', fig.height = 4, warning = F, message = F}
ggdraw(pl[[2]])
```
...
kable(cbind(se_sd_SMI), caption = "Standard error of volatility by SMI", digits = 5, format = "latex", col.names = c("Stock", "Standard error in %"), booktabs = T, linesep = "")
```

```{r tabplot6, echo = F, results="asis"}
...
kable(se_sd_groups, caption = "Standard error of volatility by group", digits = 5, format = "latex", col.names = c("Group", "Standard error in %"), booktabs = T, linesep = "")
```
...
```{r tabplot7, echo = F}
mvp_SMI = markovitz_optim_mvp(SMI_mu_sd, SMI_cov)
mvp_SMI_groups = markovitz_optim_mvp(SMI_groups_mu_sd, SMI_groups_cov)
tp_SMI = markovitz_optim_tp(SMI_mu_sd, SMI_cov)
tp_SMI_groups = markovitz_optim_tp(SMI_groups_mu_sd, SMI_groups_cov)
bs_out_SMI = bootstrap(SMI_returns)
bs_out_SMI_groups = bootstrap(SMI_groups_returns)

t7 = data.frame(n = SMI_mu_sd$Stock, m = mvp_SMI)
row.names(t7) = c()

kable(t7, caption = "Weights of MVP by stocks", digits = 3, format = "latex",
...
```{r tabplot8, echo = F}
t8 = data.frame(n = SMI_groups_mu_sd$Group, m = mvp_SMI_groups)
row.names(t8) = c()

kable(t8, caption = "Weights of MVP by group", digits = 3, format = "latex",
      col.names = c("Group", "Weight"), booktabs = T, linesep = "")
```
...
In this paper the MVP and TP serve as useful reference points to compare the Sharpe ratio of different optimizations. The tangency portfolio is often referred to in the literature as the market portfolio. In the equation for the capital market line, shown as formula \@ref(eq:cml), the expected return of the market portfolio respectively the tangency portfolio is written as $R_{tp}$.
...
```{r tabplot9, echo = F}
t9 = data.frame(n = SMI_mu_sd$Stock, m = tp_SMI)
row.names(t9) = c()

kable(t9, caption = "Weights of tangency portfolio by SMI", digits = 3, format = "latex",
      col.names = c("Stock", "Weight"), booktabs = T, linesep = "")
```


```{r tabplot10, echo = F}
t10 = data.frame(n = SMI_groups_mu_sd$Group, m = tp_SMI_groups)
row.names(t10) = c()

kable(t10, caption = "Weights of tangency portfolio by group", digits = 3, format = "latex",
      col.names = c("Group", "Weight"), booktabs = T, linesep = "")
```
...
```{r gg3, echo = F, fig.cap = 'Bootstrap samples efficiency frontier', fig.height = 8, warning = F, message = F}
text_ef1 = grobTree(textGrob("groups", x = 0.68,  y = 0.90, hjust = 0,
  gp = gpar(col = "black", fontsize = 11, fontfamily = "Times", alpha = 0.5)))

text_ef2 = grobTree(textGrob("SMI", x = 0.85,  y = 0.80, hjust = 0,
  gp = gpar(col = "black", fontsize = 11, fontfamily = "Times", alpha = 0.5)))

eff_frontier_SMI = eff_frontier(markovitz_optim_mvp(SMI_mu_sd, SMI_cov),
                            markovitz_optim_tp(SMI_mu_sd, SMI_cov), SMI_mu_sd, SMI_cov)

eff_frontier_groups = eff_frontier(markovitz_optim_mvp(SMI_groups_mu_sd, SMI_groups_cov),
                            markovitz_optim_tp(SMI_groups_mu_sd, SMI_groups_cov), SMI_groups_mu_sd, SMI_groups_cov)

SMI_mu_sd$index = rownames(SMI_mu_sd)
SMI_mu_sd$legend = paste0(SMI_mu_sd$index, ": ", SMI_mu_sd$Stock)

SMI_mu_sd$Group = c("Industrial","Consumer","Finance","Industrial","Industrial",
                    "Finance", "Industrial", "Pharma","Consumer","Pharma",
                    "Consumer","Pharma","Industrial","Pharma","Consumer",
                    "Finance","Finance","Consumer","Finance","Finance")

suppressMessages(suppressWarnings(print(ggplot() +
  geom_path(data = eff_frontier_SMI$coord_eff_frontier, aes(x = eff_frontier_sd, y = eff_frontier_mu), alpha = 0.5) +
  geom_path(data = eff_frontier_groups$coord_eff_frontier, aes(x = eff_frontier_sd, y = eff_frontier_mu), alpha = 0.5) +
  geom_point(aes_string(x = eff_frontier_SMI$coord_mvp[,1], y = eff_frontier_SMI$coord_mvp[,2], color = "'MVP'")) +
  geom_point(aes_string(x = eff_frontier_SMI$coord_tp[,1], y = eff_frontier_SMI$coord_tp[,2], color = "'TP'")) +
  geom_point(aes_string(x = eff_frontier_groups$coord_mvp[,1], y = eff_frontier_groups$coord_mvp[,2], 
                        color = "'MVP'")) +
  geom_point(aes_string(x = eff_frontier_groups$coord_tp[,1], y = eff_frontier_groups$coord_tp[,2], 
                        color = "'TP'")) +
  geom_point(data = SMI_mu_sd, aes(x = `Volatility [%]`, y = `Mean return [%]`, fill = legend), 
             alpha = 0) +
  geom_point(data = SMI_groups_mu_sd, aes(x = `Volatility [%]`, y = `Mean return [%]`), 
             color = c("lightblue4", "firebrick2", "forestgreen", "tan2"), shape = 17) +
  geom_text_repel(data = SMI_mu_sd, aes(x = `Volatility [%]`, y = `Mean return [%]`, label = index, color = Group), 
                  box.padding = 0.1, size = 2.7) +
  lims(x = c(0, 3), y = c(-0.1, 0.2)) +
  labs(x = "Volatility [%]", y = "Expected return [%]", fill = NULL, color = NULL) +
  scale_fill_discrete(breaks = SMI_mu_sd$legend) +
  scale_color_manual(values = c("MVP" = "cornflowerblue", "TP" = "orangered3",
                     ...
                     breaks = c("MVP", "TP", "Consumer", "Industrial", "Pharma", "Finance")) +
  guides(fill = guide_legend(ncol = 4), color = guide_legend(ncol = 1)) +
  theme(legend.position = "bottom",
        legend.spacing.x = unit(0, "cm")) +
  annotation_custom(text_ef1) + 
  annotation_custom(text_ef2) +
  custom_theme_markdown)))
```

In the chart above, all stocks of the SMI and the four groups are shown in a return-volatility diagram. The MVP, tangency portfolio and efficient frontier are also plotted. It is clear to see that the grouping has a lower volatility and a higher return, which leads to a better Sharpe ratio.
...
One Bootstrap sample contains a different data, with which a new mean return of each stock and group and the corresponding covariance matrix is calculated. Those two variables are needed as input in the optimization function to calculate minimum variance portfolio, tangency portfolio of the 20 stocks and the four groups. With 100 bootstrap samples, 100 MVP’s and TP’s can be compared and analyzed. Additionally, the standard deviation of the weights over those number of bootstrap samples can be studied.


```{r gg4, echo = F, fig.cap = 'Bootstrap samples efficiency frontier', fig.height = 3.8, warning = F, message = F}
suppressMessages(suppressWarnings(print(gg_bs_eff_frontier(bs_out_SMI, theme = custom_theme_markdown))))
```
...
```{r , echo = F}
min = min(c(mvp_SMI - bs_out_SMI$sd_mvp,
            mvp_SMI_groups - bs_out_SMI_groups$sd_mvp))
max = max(c(mvp_SMI + bs_out_SMI$sd_mvp,
            mvp_SMI_groups + bs_out_SMI_groups$sd_mvp))

gg3 = gg_errorbar(SMI_mu_sd$Stock, mvp_SMI,
            bs_out_SMI$sd_mvp, c(min, max), 90, "weights", theme = custom_theme_markdown)

gg4 = gg_errorbar(SMI_groups_mu_sd$Group, mvp_SMI_groups,
            bs_out_SMI_groups$sd_mvp, c(min, max), 0, "weights", theme = custom_theme_markdown)

pl = align_plots(gg3, gg4, align="h")
```

```{r gg5, echo = F, fig.cap = 'MVP weights with standard error by SMI', fig.height = 4, warning = F, message = F}
ggdraw(pl[[1]])
```

```{r gg6, echo = F, fig.cap = 'MVP weights with standard error by group', fig.height = 4, warning = F, message = F}
ggdraw(pl[[2]])
```

Figure \@ref(fig:gg5) and figure \@ref(fig:gg6) illustrate the MVP mean weights and their standard error over the 100 bootstrap samples of 20 stocks, respectively of the groups. In comparison in figure \@ref(fig:gg7) and figure \@ref(fig:gg8) the same is depicted with the tangency portfolio. As mentioned before and obvious to see is that TP weights have a higher standard deviation than MVP weights, as it was seen before. Here the standard deviation was computed with the 84-Quantil minus the 50-Quantil.

```{r , echo = F}
min = min(c(tp_SMI - bs_out_SMI$sd_tp,
            tp_SMI_groups - bs_out_SMI_groups$sd_tp))
max = max(c(tp_SMI + bs_out_SMI$sd_tp,
            tp_SMI_groups + bs_out_SMI_groups$sd_tp))

gg5 = gg_errorbar(SMI_mu_sd$Stock, tp_SMI,
            bs_out_SMI$sd_tp, c(min, max), 90, "weights", theme = custom_theme_markdown)

gg6 = gg_errorbar(SMI_groups_mu_sd$Group, tp_SMI_groups,
            bs_out_SMI_groups$sd_tp, c(min, max), 0, "weights", theme = custom_theme_markdown)

pl = align_plots(gg5, gg6, align="h")
```

```{r gg7, echo = F, fig.cap = 'tangency portfolio weights with standard error by SMI', fig.height = 4, warning = F, message = F}
ggdraw(pl[[1]])
```

```{r gg8, echo = F, fig.cap = 'Tangency portfolio weights with standard error by group', fig.height = 4, warning = F, message = F}
ggdraw(pl[[2]])
```

The TP weights of two individual stocks is illustrated in \@ref(fig:gg9). Generally the weight of the tangency portfolio spreads stongly.


```{r gg9, echo = F, fig.cap = 'Sample tangency portfolio stock weights', fig.height = 3, warning = F, message = F}
stock1 = "ABB"
stock2 = "Adecco"
...
```{r crossvalidation, echo = F, fig.cap = "Cross validation test and training sets", out.width = "95%"}
...
```{r tabplot11, echo = F}
is = in_sample(constructor(SMI_returns, SMI_stocks, SMI_groups_names, SMI_groups))
slices = slicer(SMI_returns, SMI_stocks, SMI_groups_names, SMI_groups)
os = out_of_sample(slices)

t11 = data.frame(c(is$sr_is_SMI, is$sr_is_SMI_groups),
                 c(os$sr_os_SMI, os$sr_os_SMI_groups))
rownames(t11) = c("SMI", "Groups")

kable(t11, caption = "Sharpe ratio", format = "latex", digits = 3, 
...
\begin{equation}
R_{i}(\lambda) = (1 - \lambda)*R_{i} + \lambda * \overline{R}
  \label{eq:shrinkR}
\end{equation}
...
<p> *$\rho_{i,j} (\epsilon)$ = correlation coefficient at {i,j} as a function of $\epsilon$* <p>
<p> *$\epsilon$ = shrinking factor* <p>
<p> *$I_{i,j}$ = identity matrix at {i,j}* <p>
<p> *$\tilde{\rho_{i,j}}$ = correlation matrix with diagonal zero at {i,j}* <p>
...
```{r gg20, echo = F, fig.cap = 'Sharpe ratio as a function of return shrinking factor',fig.height = 3, warning = F, message = F}
print(gg_shrinking2D(slices, r = T, theme = custom_theme_markdown))
```
...
```{r gg21, echo = F, fig.cap = 'Sharpe ratio as a function of correlation shrinking factor', fig.height = 3, warning = F, message = F}
gg_shrinking2D(slices, cor = T, theme = custom_theme_markdown)
```
...
```{r gg22, echo = F, fig.cap = 'SMI Sharpe ratio as a function of return and correlation shrinking factor', fig.height = 4, warning = F, message = F}
gg_shrinking3D(slices, theme = custom_theme_markdown)
```
...
```{r gg23, echo = F, fig.cap = 'Groups Sharpe ratio as a function of return and correlation shrinking factor', fig.height = 4, warning = F, message = F}
gg_shrinking3D(slices, groups = T, theme = custom_theme_markdown)
```
...
sr_SMI = sapply(out_of_sample_vec(slices, grid[,1], grid[,2]), function(x){return(x$sr_os_SMI)})
max_shrinking_SMI = max(sr_SMI)
sr_SMI_groups = sapply(out_of_sample_vec(slices, grid[,1], grid[,2]), function(x){return(x$sr_os_SMI_groups)})
max_shrinking_SMI_groups = max(sr_SMI_groups)

t20 = data.frame(c(is$sr_is_SMI, is$sr_is_SMI_groups),
                 c(os$sr_os_SMI, os$sr_os_SMI_groups),
                 c(max_shrinking_SMI, max_shrinking_SMI_groups))
rownames(t20) = c("SMI", "Groups")

kable(t20, caption = "Sharpe ratio", format = "latex", digits = 3,
...
The Journal of Finance Vol. 7, No. 1, pp. 77-91 (15 pages). Published By: Wiley. [Accessed 05 September 2020]
...
Published by: The University of Chicago Press [Accessed 05 September 2020]

[7] A. W. Lo (2003). The Statistics of Sharpe Ratios.
Sloan School of Management, Massachusetts Instintute of Technology, Cambridge. [Accessed 03 November 2020]

[8] Ahn, S. and Fessler, J. (2003). Standard Errors of Mean, Variance and, Standard Deviation Estimators. 
EECS Department, The University of Michigan, pp.1-2. [Accessed 05 October 2020]

[9] D. G. Leuenberger (1998). Investment Science. 
Published by: Oxford University Press, New York [Accessed 25 September 2020]
