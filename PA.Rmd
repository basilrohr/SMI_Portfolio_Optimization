...
It can already be seen that the values of volatility are lower overall than those of the individual shares. In the matrix below, the correlation of the individual groups is shown. As the returns are averaged within the groups, the values of the individual groups converge, resulting in a larger correlation coefficient.

```{r, fig.cap = "Groups correlation", fig.height = 5}
suppressWarnings(suppressMessages(print(gg_cor(cor_mat(gr), 4, 12, theme = custom_theme_markdown) + 
  scale_fill_gradient2(low = "cornflowerblue", high = "orangered2", mid = "honeydew", limit = c(-1,1), name = "Cor"))))
```


### Mathematical grouping
In a second approach the grouping is made my sorting the euclidean distance. The group formation is based on the similarity measure, which is calculated here with Euclidean distances between two correlation coefficients. The formula is illustrated below.
...
The Formula takes all column values of a first stock and calculates the squared difference to the column values of the second stock. These 20 values are summed up and the root is extracted from them. The result is the distance between the two stocks, and if it is done for all 20 stocks it results in a distance matrix. The algorithm sorts the values of the distance matrix with the method “complete”.
The first step is (independent of methods) grouping the two stocks with the smallest distance. 
...
For distances between groups, the maximum value is taken from the distances between each object of a group with the object of another group.
For distance between group ($x_{1},x_{2}$) and ($x_{3},x_{4}$) with two objects (stocks) applies \newline
$d\langle \{x_{1},x_{2}\} , \{x_{3},x_{4}\} \rangle = max(d\langle x_{1}, x_{3} \rangle, d\langle x_{1}, x_{4} \rangle, d\langle x_{2}, x_{3} \rangle, d\langle x_{2}, x_{4} \rangle)$. The third step is repeating the procedure with the new created distance matrix until only two groups remain. The outcome is a dendogram which is also part of the illustration in \@ref(fig:heatmap). It can be seen that the first step of grouping is the connection which is drawn lowest.
...
## Bootstrap
Bootstrapping is a statistical method of resampling. It allows to create a new sample with drawing from the existing sample. With 100 or more resampled data, each mean and variance of the samples can be analyzed. This method can be used to draw conclusion about a population from a sample.
...
One Bootstrap sample contains a different data, with which a new mean return of each stock and group and the corresponding covariance matrix is calculated. Those two variables are needed as input in the optimization function to calculate minimum variance portfolio, tangency portfolio of the 20 stocks and the four groups. With 100 bootstrap samples, 100 MVP’s and TP’s can be compared and analyzed. Additionally, the standard deviation of the resulting weights over those number of bootstrap samples can be studied. The standard deviation is computed with the 84-Quantil minus the 50-Quantil to achieve a more robust result, because this calculation is less vulnerable to outliers.
...
In this case, the data set is split into five sections and consequentially five models are trained as depicted in figure \@ref(fig:crossvalidation). The gray sections are the training sets and the red ones the test sets.
...
At the end of the cross validation, all five test sets are combined into one time series consisting of the daily weighted returns and equaling the length of the original data set. This time series is completely out of sample and is used to calculate the Sharpe ratio and hence measure the performance of the model.

It is to note that the training and test sets of the five sections vary significantly in some cases. To give one cause for this, the financial crisis from 2007–2008 can be looked at. During this period, most assets yielded negative returns. For example, model 3 in figure \@ref(fig:crossvalidation) includes this time period as part of the test set and model 4 of the training set. This results in fluctuating asset weights across the five models which are illustrated in figure \@ref(fig:tpwbymodel).

```{r tpwbymodel, fig.cap = "Tangency portfolio weights by model", fig.height = 5}
g2r = groups_returns(r, groups2); g2mr = mean_returns(g2r); g2cm = cov_mat(g2r)
...
The mean returns shrinkage factor is calculated as in \@ref(eq:shrinkr).
...
\begin{equation}
\lambda = \frac{-\vec{1}\Sigma^{-1}\vec{\overline{R}}}{\vec{1}\Sigma^{-1}\vec{R} - \vec{1}\Sigma^{-1}\vec{\overline{R}})}
\label{eq:lambdazero}
\end{equation}

If the zero crossing is in the shrinkage interval of $[0, 1]$ this results in extreme asset weights. As the denominator approaches zero, the scaling factor becomes very large which carries over to the weights. To illustrate this effect, the weights of the five cross validation models from figure \@ref(fig:crossvalidation) are plotted as a function of $\lambda$ in figure \@ref(fig:tpwshrinkbymodel).
...
Model three shows such extreme asset weights. The asymptote is visualized as a vertical red line. These extreme weights influence the out of sample time series and consequentially also the out of sample Sharpe ratio. The other four models do show asset weights in a normal scale.

The scaling factor as a function of $\lambda$ is shown in \@ref(fig:tpwsfshrinkbymodel).
...
The zero crossing of the scaling factor in model three can be seen clearly. When working with mean return shrinkage one has to be aware of this behavior.

\newpage

### Shrinkage of correlation matrix

The correlation matrix shrinkage factor is calculated as in \@ref(eq:shrinkcor).
...
# Results
In this section the different approaches which were described in chapter \@ref(methodology) methodology are applied on the historical data. Section I. optimizes the entire SMI with 20 stocks. Section II. includes the optimization of the groups. Section III. applies the shrinkage.
...
```{r tabplot12}
t12 = data.frame(is_gr, os_gr, row.names = "Groups")

kable(t12, caption = "Sharpe ratio",  digits = 3, 
       booktabs = T, linesep = "", col.names = c("In sample", "Out of sample"))
```
Here in table \@ref(tab:tabplot12) the sharpe ratio of in-and out-of-sample of the formed groups are presented. The first observation is, that the in-sample value has become smaller after this method was applied. The reason for this is, that there are less parameters to optimize with the grouping method. Instead of 20 different stocks there are now only 4 stocks respectively groups which can be optimized. That means with less parameters or data, less portfolio variations can be made to find the best possible optimization.
However, the out-of-sample sharpe ratio has increased. This is because with forming groups, stock returns of companies within the group are being averaged. Consequently, the group return is more stable over the time period. As described more detailed in results \@ref(resi), this leads to a better performance respectively a better sharpe ratio. Therefore, also in- and out-of-sample are here closer together.
...
```{r gg20, fig.cap = 'Sharpe ratio as a function of return shrinking factor',fig.height = 4}
...
gg_shrinking2D(os_r_sr, os_gr_sr, os_g2r_sr, "SMI", "Groups 4", "Groups 3", "Return", theme = custom_theme_markdown)
```
...
```{r gg21, fig.cap = 'Sharpe ratio as a function of correlation shrinking factor', fig.height = 4}
...
gg_shrinking2D(os_r_scor, os_gr_scor, os_g2r_scor, "SMI", "Groups 4", "Groups 3", "Correlation", theme = custom_theme_markdown)
```
...
Figure \@ref(fig:gg23) shows the three dimensional plot for the groups. Here, a much smaller fraction of the plot area shows Sharpe ratios which are similarly high. The highest Sharpe ratio is achieved at a correlation shrinking factor of close to one and a return shrinking factor of zero. Varying both shrinking factors toghether yields also the same Sharpe ratio as in \@ref(fig:gg21) were only the correlation shrinking factor is varied.
...
```{r gg24, fig.cap = 'Groups Sharpe ratio as a function of return and correlation shrinking factor', fig.height = 4}
os_g2r_sr_scor = unlist(out_of_sample_vec(sets_g2r, grid[,1], grid[,2]))

gg_shrinking3D(grid, os_g2r_sr_scor, theme = custom_theme_markdown)
```
