...
The matrix in figure \@ref(fig:corplot) shows the correlations of all 20 stocks included in the SMI. By definition, the diagonal values are always 1. As the SMI only contains shares that are traded on the Swiss stock exchange, there is a rather high correlation between the individual companies. The correlation between companies from the same sector is also higher than that between companies from different sectors or which have less in common. From this it can be concluded that economic similarities are reflected in a higher correlation of stock returns. Another way to demonstrate a connection between two companies is the t-statistic which is described in chapter \@ref(t-statistic).
...
In the plot above (figure \@ref(fig:serplot)) you can see the logarithmic mean return with the standart errors for each stock. It is easy to see that stocks such as *Swisscom* and *Nestle* with low volatility also have a lower standard error. As well *ABB* and *Credit Suisse* show a high volatility and accordingly have higher values for the standard error. Figure \@ref(fig:ser) and table \@ref(tab:rmrvoltab) can therefore be compared well together.
...
As shown in table \@ref(tab:sevolrtab) the values of the standard error of volatility are much lower as the one of the mean returns. This behavior will be clearly visible later in the chapter bootstrap. Furthermore, we will neglect the values for any further calculations.
**wir können die Volatilität praktisch exakt schätzen, als vernachlässigen wir den Schätzfehler im folgenden**
...
**Markowitz optimization assumes that the return … is known.**
...
plot vermutlich loeschen da nicht mehr relevant
...
**: negative return bedeutet nicht unbedingt negatives Gewicht und umgekehrt. Es kommt noch auf die Korrelationen an**
...
**text umschreiben, SMI nicht erwaehnen**
...
**unterschiedliche gruppengroessen beschreiben und was das bedeutet**
**eine 3er und eine 11er gruppe eher wenig sinnvoll**

**plot von eingefaerbten gruppen zeigen analog shiny**
...
## Sharpe ratio of SMI as a portfolio {#res1}
...
## Results of grouping and bootstrapping method {#res2}
...
gg_shrink2D(list(os_r_sr, os_gr_sr, os_g2r_sr), c("SMI", "Groups by industry", "Groups by correlation"), "Return", theme = custom_theme_markdown)
```
...
gg_shrink2D(list(os_r_scor, os_gr_scor, os_g2r_scor), c("SMI", "Groups by industry", "Groups by correlation"), "Correlation", theme = custom_theme_markdown)
```
