...
abstract: "Markowitz optimization requires estimates of the expected return, standard deviation, and correlations of individual stocks, all of which are subject to estimation errors. This leads to the risk of overfitting historical data and, in the case of portfolio optimization, creating portfolios that perform well in backtesting but poorly in the future. To overcome these problems, several robust approaches are investigated, such as different asset grouping methods and shrinkage factors for historical returns and correlations. The goal is to provide a recommended methodology for optimizing a portfolio that includes all assets from the Swiss Market Index. In summary, two key observations are made. First, asset grouping leads to better portfolio performance relative to individual assets. Similarly, shrinkage leads to increased performance for assets with noisy historical data.\\par\\textbf{Keywords:} Portfolio optimization, Robust optimization, Markowitz model, Covariance matrix, Shrinkage, Sharpe ratio\\newpage"
output:
  bookdown::pdf_book:
    toc: false
...
<p> *$\vec{w}_{tp}$ = weights of tangency portfolio* <p>
...
## Grouping {#grouping}
Grouping of stocks is an approach to reduce the noise given by individual assets and therefore increase the Sharpe ratio. A qualitative and a mathematical procedure are applied.
...
## Bootstrap {#bootstrap}
Bootstrapping is a statistical method of resampling. It allows to create a new sample with drawing from the existing sample. With 100 or more resampled data, each mean and variance value of the samples can be analyzed. This method can be used to draw conclusion about a population from its sample.
...
**Step 1**
The dates of the historical data are sampled (using `sample()`) equaling its length, with replacing included. In other words, the row numbers of the historical data are sampled. The argument `replace = TRUE` ensures, that certain dates, respectively rows can occur multiple times. If replacing would not be included in the sampling method, the correlation matrix would remain constant to the one of the original data.
...
Before the results of the optimization are shown for all individual assets, the errors of the bootstrapped weights are considered in detail. In the graphics the mean weights and the standard deviation over the 100 samples are shown. Figure \@ref(fig:mvpse) depicts the MVP weights of the 20 stocks and figure \@ref(fig:tpse) the TP weights.
...
```{r tpse, fig.cap = 'TP weights with standard error', fig.height = 4}
...
It is obvious to see in figure \@ref(fig:mvpse) and \@ref(fig:tpse) and also visualized before in figure \@ref(fig:gg4) (chapter \@ref(bootstrap)), that TP weights have a higher standard deviation than MVP weights. That means the TP is more sensible to changes of returns than the MVP. If the eq. \@ref(eq:mvpw) of MVP and the eq. \@ref(eq:tpw) of TP are considered, the MVP does not depend on the returns directly. Consequently different returns samples have not a large impact on the MVP. The small differences arise from the varying covariance matrix. But it can also be noticed that this matrix has not large deviations following the standard deviation of the MVP. Therefore bootstrap sampling of the returns does not influence the covariance matrix as much as the returns itself.

Table \@ref(tab:tabplot11) compares the in and out of sample Sharpe ratio for all 20 stocks.

```{r tabplot11}
t11 = data.frame(is_r, os_r, row.names = "SMI constituents")
kable(t11, caption = "Sharpe ratio SMI constituents",  digits = 3, 
       booktabs = T, linesep = "", col.names = c("In sample", "Out of sample"))
```

In sample, training and test set are equal and therefore its Sharpe ratio is higher than out of sample. This is because the model performs best on the data which built it. Out of sample, training set and test set differ. If a stock behaves differently in one test set compared to the training sets used to built the model, this stock can easily be over- or undervalued. This leads to a worse performance of the model and hence the Sharpe ratio. The conclusion that can be drawn from this insight is, that the in sample Sharpe ratio should always be higher than the out of sample Sharpe ratio.

\newpage

## Results of grouped SMI constituents {#res2}
As mentioned in section \@ref(res1), the errors from the bootstrapping method for grouped assets are presented before their Sharpe ratio performance. Figure \@ref(fig:mvpsegr) illustrates the MVP weights and figure \@ref(fig:tpsegr) the TP weights.

```{r mvpsegr, fig.cap = 'MVP weights with standard error by group', fig.height = 3}
gg_errorbar(colnames(gr[-1]), mvpw_gr, mvpw_sd_gr, c(minmvp, maxmvp), "Weights",
            theme = custom_theme_markdown)
```

```{r tpsegr, fig.cap = 'TP weights with standard error by group', fig.height = 3}
gg_errorbar(colnames(gr[-1]), tpw_gr, tpw_sd_gr, c(mintp, maxtp), "Weights",
            theme = custom_theme_markdown)
```

The same conclusion can be drawn for weights of grouped assets as in \@ref(res1).
...
Figure \@ref(fig:scorplot) shows the same plot for the correlation shrinkage factor. There is a clear maximum for correlation shrinkage factor 0.4. For the SMI constituents, the same observation can be made as in figure \@ref(fig:srplot). The out of sample Sharpe ratio drops, if past correlations are taken too seriously (higher shrinkage factor) or less seriously (shrinkage factor 0). Also, the result for groups is similar to the previous one that shrinkage does not have a great impact. The highest Sharpe ratio is again achieved by the groups by correlation.
...
To give further insights, the tangency portfolio weights for unshrunk and shrunk mean returns and correlations are compared in tables \@ref(tab:tpwrstab), \@ref(tab:tpwgrtab) and \@ref(tab:tpwg2rtab). The weights of unshrunk mean returns are exactly the ones used to calculate the in sample Sharpe ratio. The Shrinkage factors utilized for the shrunk mean returns and correlations are the ones which obtained the highest out of sample Sharpe ratio.

The weights before shrinkage in the following table \@ref(tab:tpwrstab) can be found in the previous chapter \@ref(markovitz) in table \@ref(tab:tpwrtab).
...
kable(tpw_gr_df, col.names = c("Group", "before shrinkage", "after shrinkage"), caption = "Groups by industry in sample tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
...
kable(tpw_g2r_df, col.names = c("Group", "before shrinkage", "after shrinkage"), caption = "Groups by correlation in sample tangency portfolio weights",
      digits = 3, booktabs = T, linesep = "")
```
