...
```{r gcacm, fig.cap = "Groups correlation", fig.height = 5}
...
```{r tabplot15}
t15 = data.frame(c(is_gr, is_g2r),
                 c(os_gr, os_g2r), row.names = c("Groups 4", "Groups 3"))

kable(t15, caption = "Sharpe ratio",  digits = 3,
       booktabs = T, linesep = "", col.names = c("In sample", "Out of sample"))
```

In chapter \@ref(groupclust), cluster analysis is used to optimize the group collocation. With that more correlating stocks are put together to have a more diversifying portfolio. As it can be extracted directly from the two correlation matrices from grouping by industry (figure \@ref(fig:gcm)) and grouping by correlation (figure \@ref(fig:gcacm)), the correlation between the groups shrunk as expected. That means the grouping by correlation was successful. Further seen in table \@ref(tab:tabplot15)), the Sharpe ratio of the new grouping method is much higher than grouping by industries which is a logical consequence of the diversification. In reality though, it is unlikely to invest in such a compiled portfolio, however it was made to find a method for grouping which affected the Sharpe ratio positively.
