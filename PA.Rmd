...
m3 = solve(sets_r[[1]][[3]]$cov_mat); r3 = sets_r[[1]][[3]]$mean_returns
denom_zero = (- rowSums(m3) %*% rep(mean(r3), n)) / (rowSums(m3) %*% r3 - rowSums(m3) %*% rep(mean(r3), n))
...
### Shrinking of correlation matrix
...
# Results
In this section the different approaches which were desribed in chapter methodology \@ref(methodology) are applied on the historical data. Section I. optimizes the entire SMI with 20 stocks. Section II. includes the optimization of the groups. Section III. applies the shrinkage.
...
```{r gg5, fig.cap = 'MVP weights with standard error', fig.height = 4}
...
```{r gg7, fig.cap = 'tangency portfolio weights with standard error', fig.height = 4}
...
```{r gg6, fig.cap = 'MVP weights with standard error', fig.height = 4}
