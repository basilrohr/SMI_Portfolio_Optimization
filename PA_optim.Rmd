...
kable(ann_rf, caption = "Average overnight risk-free return per year in \\%", 
...
kable(r_mr_vol, caption = "SMI with expected return and volatility", digits = 3, 
...
kable(gr_mr_vol, caption = "groups with expected return and volatility", col.names = c("Group","Return [%]", "Volatility [%]"),  digits = 3, booktabs = T, linesep = "")
```
...
gg1 = gg_errorbar(colnames(r[-1]), mr, se_r, c(min, max), "Return [%]", theme = custom_theme_markdown)
gg2 = gg_errorbar(colnames(gr[-1]), gmr, se_gr, c(min, max), "Return [%]", theme = custom_theme_markdown)

pl = align_plots(gg1, gg2, align="h")
```

```{r gg1, fig.cap = 'Expected mean return with standard error by stock', fig.height = 4}
ggdraw(pl[[1]])
```

```{r gg2, fig.cap = 'Expected mean return with standard error by group', fig.height = 4}
ggdraw(pl[[2]])
```
...
kable(se_volr, caption = "Standard error of volatility by SMI", digits = 5,  col.names = c("Stock", "Standard error in %"), booktabs = T, linesep = "")
```

```{r tabplot6}
se_volgr = data.frame(colnames(gr[-1]), apply(gr[,-1], 2, se_sd), row.names = NULL)

kable(se_volgr, caption = "Standard error of volatility by group", digits = 5,  col.names = c("Group", "Standard error in %"), booktabs = T, linesep = "")
```
...
kable(mvpw_r, caption = "Weights of MVP by stocks", digits = 3, 
...
kable(mvpw_gr, caption = "Weights of MVP by group", digits = 3, 
...
kable(tpw_r, caption = "Weights of tangency portfolio by SMI", digits = 3, 
      col.names = c("Stock", "Weight"), booktabs = T, linesep = "")
```
...
kable(tpw_gr, caption = "Weights of tangency portfolio by group", digits = 3, 
      col.names = c("Group", "Weight"), booktabs = T, linesep = "")
```
...
```{r gg3, fig.cap = 'Bootstrap samples efficiency frontier', fig.height = 8}
...
gg3 = gg_errorbar(colnames(r[-1]), mvpw_r, mvpw_sd_r, c(min, max), "Weights", theme = custom_theme_markdown)
...
gg3 = gg_errorbar(colnames(r[-1]), tpw_r, tpw_sd_r, c(min, max), "Weights", theme = custom_theme_markdown)

gg4 = gg_errorbar(colnames(gr[-1]), tpw_gr, tpw_sd_gr, c(min, max), "Weights", theme = custom_theme_markdown)
...
os_r_sr = out_of_sample_vec(sets_r, seq(0, 1, 0.01))
os_gr_sr = out_of_sample_vec(sets_gr, seq(0, 1, 0.01))
...
os_r_scor = out_of_sample_vec(sets_r, 0, seq(0, 1, 0.01))
os_gr_scor = out_of_sample_vec(sets_gr, 0, seq(0, 1, 0.01))
...
os_r_sr_scor = out_of_sample_vec(sets_r, grid[,1], grid[,2])
...
os_gr_sr_scor = out_of_sample_vec(sets_gr, grid[,1], grid[,2])
