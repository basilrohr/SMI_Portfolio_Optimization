# This function performs the Markovitz optimization with covariance matrix inversion.
# The inputs are a data frame with the column mean return,
# the corresponding covariance matrix and the risk-free rate.
# The outputs are the weights of the minimum variance portfolio and tangency portfolio.

markovitz_optim = function(df, cov, Rf = 0) {
  
  # Calculate minimum variance portfolio weights
  ones = rep(1, nrow(cov))
  mu_inv = ones %*% solve(cov) %*% ones
  sd_mu_min_weights = c(1/mu_inv) * solve(cov) %*% ones
  
  # Calculate tangency portfolio weights
  r = df[,2]
  lambda_inv = ones %*% solve(cov) %*% (r - c(Rf %*% ones))
  tang_port_weights = c(1/lambda_inv) * solve(cov) %*% (r - c(Rf %*% ones))
  
  return(c(data.frame(sd_mu_min_weights), data.frame(tang_port_weights)))
}
